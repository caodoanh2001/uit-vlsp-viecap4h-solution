...
class TextField(RawField):
    ...
    def process(self, batch, device=None):
        ...
        # print('subwords', subwords.shape)
        # print('lens', lens.shape)

        # input_ids_bert, masks, lens

        # phobert_tokenizer = PhobertTokenizer.from_pretrained("vinai/phobert-base")
        # dict_stoi_bert = {value: bert_tokenizer.encode(key)[1:-1] for key, value in dict(self.vocab.stoi).items()}
        ...
        #   ids = [dict_stoi_bert[s] for s in sequence]
        #   flatten_ids = sum(ids, [])
        ...
        #   input_ids_bert['flatten'][i] = ids + [1]*(ori_max_len-len(ids))
        #   # lens.append([len(piece) for piece in input_ids_bert['sub'][i]] + \
        #   #  [1]*(max_len-len(ids)))
        #   temp_lens = [len(piece) for piece in input_ids_bert['sub'][i]] + \
        #    [0]*(max_len-len(ids))
        #   lens.append(temp_lens)

        # input_ids_bert = torch.tensor(input_ids_bert['flatten'])
        # masks = torch.ones_like(input_ids_bert).long()
        # masks = masks.gt(0)
