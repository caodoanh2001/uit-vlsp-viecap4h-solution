...
def evaluate_metrics(model, dataloader, text_field):
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (images, caps_gt) in enumerate(iter(dataloader)):
            ...
            with torch.no_grad():
                out, _ = model.beam_search(images, 20, text_field.vocab.stoi['</s>'], 5, out_size=1)
            caps_gen = text_field.decode(out, join_words=False)
            for i, (gts_i, gen_i) in enumerate(zip(caps_gt, caps_gen)):
                gen_i = ' '.join([k for k, g in itertools.groupby(gen_i)])
                gen['%d_%d' % (it, i)] = [gen_i.replace('_', ' '), ]
                gts['%d_%d' % (it, i)] = gts_i
            pbar.update()
...
def train_scst(model, dataloader, optim, cider, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, caps_gt) in enumerate(dataloader):
            # detections, caps_gt, input_ids_bert, masks, lens = \
            #       detections.to(device), caps_gt.to(device), input_ids_bert.to(device), masks.to(device), lens.to(device)
            
            detections = detections.to(device)
            outs, log_probs = model.beam_search(detections, seq_len, text_field.vocab.stoi['</s>'],
            ...
            #caps_gen = {key: [caps_gen[key][0].replace('_', '')] for key in caps_gen}
            new_caps_gt = {}
            for batch in caps_gt:
              new_caps_gt[batch] = []
              for sentence in caps_gt[batch]:
                new_sentence = word_tokenize(sentence)
                new_sentence = [s.replace(' ', '_') for s in new_sentence]
                new_sentence = ' '.join(new_sentence)
                new_caps_gt[batch].append(new_sentence)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--xe_most', type=int, default=22)
    parser.add_argument('--refine_epoch_rl', type=int, default=28)

    parser.add_argument('--xe_base_lr', type=float, default=0.0005)
    ...
    # Pipeline for text
    text_field = TextField(init_token='<s>', eos_token='</s>', lower=True, tokenize='spacy', remove_punctuation=True, nopoints=False)

    # Create the dataset
    dataset = COCO(image_field, text_field, args.img_path, args.annotation_folder, args.annotation_folder)
    ...
    print('len vocab:', len(text_field.vocab))
    ...
    #optim = SGD(model.parameters(), lr=1, momentum=0.9)
    #scheduler = scheduler = LambdaLR(optim, lambda_lr)
    ...
    dir_save_models = 'saved_transformer_models_15_10_2021_4layers'
    if not os.path.exists(dir_save_models):
        os.mkdir(dir_save_models)

    if args.resume_last or args.resume_best:
        if args.resume_last:
            fname = dir_save_models + '/%s_last.pth' % args.exp_name
        else:
            fname = dir_save_models + '/%s_best.pth' % args.exp_name
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if switch_to_rl and not best:
            data = torch.load(dir_save_models + '/%s_best.pth' % args.exp_name)
            torch.set_rng_state(data['torch_rng_state'])
            torch.cuda.set_rng_state(data['cuda_rng_state'])
            np.random.set_state(data['numpy_rng_state'])
            random.setstate(data['random_rng_state'])
        ...
        torch.save({
            ...
            'use_rl': use_rl,
        }, dir_save_models + '/%s_last.pth' % args.exp_name)

        
        if e == 21:
            copyfile(dir_save_models + '/%s_last.pth' % args.exp_name, dir_save_models + '/%s_best_rl_21.pth' % args.exp_name)

        if best:
            copyfile(dir_save_models + '/%s_last.pth' % args.exp_name, dir_save_models + '/%s_best.pth' % args.exp_name)
        if best_test:
            copyfile(dir_save_models + '/%s_last.pth' % args.exp_name, dir_save_models + '/%s_best_test.pth' % args.exp_name)

        # 保存模型，用于微调
        if e >= 25:
            copyfile(dir_save_models + '/%s_last.pth' % args.exp_name, dir_save_models + '/{}_{}.pth'.format(args.exp_name, e))
