    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader)) as pbar:
        with torch.no_grad():
            for it, (detections, captions, input_ids_bert, masks, lens) in enumerate(dataloader):
                ...
                out, _ = model(captions, input_ids_bert, masks, lens)
...
def train_xe(model, dataloader_train, optim, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader_train)) as pbar:
        for it, (detections, captions, input_ids_bert, masks, lens) in enumerate(dataloader_train):
            detections, captions, input_ids_bert, masks, lens = \
              detections.to(device), captions.to(device), input_ids_bert.to(device), masks.to(device), lens.to(device)
            # out = model(detections, captions)
            # truoc khi vao model
            ...
            #print(out.view(-1, len(text_field.vocab)))
...
if __name__ == '__main__':
    ...
    if not os.path.exists('vocab_language'):
        os.mkdir('vocab_language')

    if not os.path.isfile('vocab_language/vocab_%s.pkl' % args.exp_name):
        print("Building vocabulary")
        text_field.build_vocab(train_dataset, val_dataset, min_freq=args.min_freq)
    ...
    if not os.path.exists('saved_language_models'):
        os.mkdir('saved_language_models')
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        # Test scores
        test_scores = evaluate_metrics(model, dataloader_val, text_field)
