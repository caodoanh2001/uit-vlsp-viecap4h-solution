...
class TransformerDecoderLayer(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, language_model_path='./saved_language_models/bert_language_best_test.pth', d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, bert_hidden_size=768, dropout=.1,
    ...
    def forward(self, input, input_ids_bert, bert_mask, lens, encoder_output, mask_encoder, pos):
        # input (b_s, seq_len)
        b_s, seq_len = input.shape[:2]
        _, bert_seq_len = input_ids_bert.shape[:2]
        mask = lens.gt(0)
        ...
        _, language_feature = self.language_model(input, input_ids_bert, bert_mask, lens)
